...
export async function GET(req: Request) {
  ...
  if (tipo === 'segmentacion') {
    const { data: isLeader } = await vigilar('app/api/reportes/fn_is_ministry_leader', supabase.rpc('fn_is_ministry_leader'));
    if (!isLeader) return NextResponse.json({ error: 'No autorizado' }, { status: 403 });
  } else {
    const { data: isAdmin } = await vigilar('app/api/reportes/fn_is_admin', supabase.rpc('fn_is_admin'));
  ...
  if (tipo === 'inscripciones') {
    ...
    const { data } = await vigilar('app/api/reportes', q);
    ...
    const { data } = await vigilar('app/api/reportes/certificates', supabase.from('certificates').select('full_name,course_name,completion_date,status,verify_code,issued_at, profiles!certificates_user_id_fkey(email)'));
    ...
    const { data } = await vigilar('app/api/reportes/dream_team_forms', supabase.from('dream_team_forms')
    ...
    const { data } = await vigilar('app/api/reportes/ministry_assignments', supabase.from('ministry_assignments').select('status,notes,created_at, ministries(name), profiles!ministry_assignments_user_id_fkey(first_name,last_name,email)'));
    ...
    const { data } = await vigilar('app/api/reportes/profiles', supabase.from('profiles').select('first_name,last_name,email,phone,city,state,account_status,created_at'));
